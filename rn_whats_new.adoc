...
[role="lead"]
NetApp 服务引擎 2.7 引入了以下功能，以支持 NetApp Keystone 灵活订阅（ Flex 订阅）的增强服务：



== NetApp 服务引擎 2.7 中引入的功能

此版本中的新增功能包括：



=== 可以通过 Flex订 阅查看 Cloud Volumes ONTAP 使用情况和计费

如果您是 NetApp Keystone Flex 订阅客户且同时拥有 Cloud Volumes ONTAP 订阅，则可以通过 Flex 订阅查看云卷上的使用情况和计费。Cloud Manager 现在可以发现您的 Flex 订阅详细信息并在 Cloud Manager 界面上显示这些详细信息。单击 * 所有服务 > 数字电子钱包 > Keystone Flex 订阅 * 选项卡后，您可以执行以下活动：
...
* 将您的 Cloud Volumes ONTAP 帐户与您的 Flex 订阅 ID 关联起来，以便为计费和容量使用情况报告关联此帐户。
...
NOTE: NetApp Keystone Flex 订阅 2.7 的发布版本是 2.2 版之后的主要版本。2.2 和 2.7 之间的内部版本仅提供了一些次要修复。
